...
<head>
  ...
  <style>
    ...
    *::after {
    ...
    .check-wrap:has(input:checked) .check-box {
      background: var(--teal);
      border-color: var(--teal);
    }

    .check-wrap:has(input:checked) .check-box::after {
      content: '✓';
      font-size: 11px;
      font-weight: 800;
      color: #fff;
    ...
    .btn-success {
      background: linear-gradient(135deg, #10b981, #0da876) !important;
      ...
      <div class="auth-box">
        ...
        <div class="type-toggle">
          <button class="active" id="btnBuyer" onclick="setType('buyer')"><i class="bi bi-person-heart"></i>
            Buyer</button>
          <button id="btnBusiness" onclick="setType('business')"><i class="bi bi-briefcase"></i> Business</button>
        </div>
        ...
        <form method="POST" action="{{ route('login') }}" id="loginForm">
          ...
          <div class="field">
            ...
            @error('email')
            <div class="field-err" style="display:flex"><i class="bi bi-exclamation-circle"></i> {{ $message }}</div>
            @enderror
          </div>
